	...
	WebPlayerBridge.prototype.findPotPlayer = function() {
		if (!this.hwnd) {
			...
			,	run: function() {
					...
					this.window = window.open(location.href.substring(0, location.href.lastIndexOf("/")) + "/bridge/player.html?241213", "player", "scrollbars=no,location=no");
					if (this.window) {
...
}
{
	...
	player.run = function() {
		...
		this.window = window.open(location.href.split("index.html?241213")[0] + "view/player.html?241213", "player", "scrollbars=no,location=no");
		if (this.window) {
			this.window.document.title = "플레이어";
		}
	}
