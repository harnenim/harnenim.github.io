...
function setSetting(setting, initial=false) {
	...
	if (initial
	 ...
	 || (JSON.stringify(oldSetting.highlight) != JSON.stringify(setting.highlight))
		...
		$.ajax({url: "lib/Jamaker.color.css?251213"
			,	dataType: "text"
			,	success: (preset) => {
	...
	if (initial || (oldSetting.size != setting.size)) {
		$.ajax({url: "lib/Jamaker.size.css?251213"
			,	dataType: "text"
				,	success: (preset) => {
...
function openSetting() {
	SmiEditor.settingWindow = window.open("setting.html?251213", "setting", "scrollbars=no,location=no,resizable=no,width=1,height=1");
...
function openHelp(name) {
	const url = (name.substring(0, 4) == "http") ? name : "help/" + name.replaceAll("..", "").replaceAll(":", "") + ".html?251213";
...
SmiEditor.Addon = {
	...
	,	open: function(name, target="addon") {
			...
			const url = (name.substring(0, 4) == "http") ? name : "addon/" + name.replaceAll("..", "").replaceAll(":", "") + ".html?251213";
	...
	,	openExtSubmit: function(method, url, values) {
			...
			this.windows.addon = window.open("addon/ExtSubmit.html?251213", "addon", "scrollbars=no,location=no,width=1,height=1");
			setTimeout(() => {
